...
- 会话排行支持打开 session 文件或复制路径，方便排查高消耗任务。
- Codex 任务完成时播放完成提示音。
- 检测到失败、受阻、超时或取消时播放失败提示音。
- 提示音完全本地播放，不依赖 macOS 通知声音是否生效。
- 设置窗口支持登录时启动开关、安静时段、菜单栏显示模式、用量阈值和自定义完成/失败声音。
...
设置里的登录时启动优先使用 macOS 系统登录项；源码脚本安装登录项时会写入当前用户的 `~/Library/LaunchAgents/com.whitewood.codex-monitor.plist`，不会修改系统级目录。
...
- Session ranking actions to open a session file or copy its path when investigating high-usage tasks.
...
- Preferences window for launch at login, quiet hours, menu bar display mode, usage thresholds, and custom completion/failure sounds.
