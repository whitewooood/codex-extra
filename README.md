...
- Preferences 窗口支持登录时启动开关、安静时段、菜单栏显示模式、用量阈值和自定义完成/失败声音。
- 可选自动检查 GitHub Releases，并在发现新版本时提醒手动下载。
...
- Optional GitHub Releases update checks with a manual download prompt when a new version is available.
