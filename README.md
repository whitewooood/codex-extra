...
一个 macOS 菜单栏小工具，用来监听 Codex Desktop 的会话日志，显示本地 token 用量与限额窗口，并在任务完成或失败时播放本地声音。

菜单栏图标旁会显示两条单色微型剩余额度条：上方为 5 小时窗口，下方为 7 天窗口。
下拉面板以 Codex 用量为主，声音提醒作为辅助设置；提醒音不依赖 macOS 通知系统。

## 功能

- 监听 `~/.codex/sessions/**/*.jsonl`。
- 菜单栏图形用量条会随最新 `token_count` 事件自动刷新。
