...
- Codex 请求批准提权操作时播放独立的“批准提醒”提示音，默认开启并遵守总提醒开关和安静时段。
- 设置页新增“批准提醒”声音开关、试听和自定义声音文件，诊断页同步显示批准声音状态。

## [0.3.18] - 2026-06-17

### 新增

- 设置页新增失败判定模式：严格、智能、开发者；默认使用严格模式，减少文本启发式和命令退出码带来的误判。

### 修复

- 任务提醒只在匹配到有效 `task_started` 后处理对应的新鲜 `task_complete`，避免重扫历史日志、孤立完成事件或重复读取同一任务时误播放提示音。

## [0.3.17] - 2026-06-17

### 变更

- 用量额度行增加“距离重置还有多久”的倒计时，并把剩余额度、具体重置时间分成两行显示。

## [0.3.16] - 2026-06-15

### 修复

- 重新安装或本地更新后会等待旧菜单栏进程完全退出，再启动新版本，减少残留图标点击无响应。
- App 启动时会清理同 bundle 的旧实例，避免多个菜单栏实例同时存在。

## [0.3.15] - 2026-06-15

### 修复

- 用量窗口重置时间跨天时显示日期和时间，避免 7 天窗口只显示 `HH:mm` 造成日期不明确。

## [0.3.14] - 2026-06-15

### 修复

- 调整 DMG Finder 布局，给隐藏的 `.background` 文件夹设置独立位置，避免用户开启“显示隐藏文件”时遮挡安装标题。
- 下移并加长 DMG 背景箭头，使箭头更接近应用图标和 Applications 落点的视觉中心。
