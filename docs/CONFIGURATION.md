...
扫描频率会根据状态自动调整：有活跃 Codex 任务时每 1.5 秒扫描一次，空闲时每 6 秒扫描一次。最近会话文件发现每 12 秒刷新一次，全量会话文件发现每 60 秒刷新一次；文件内容采用增量读取，只读取上次扫描后的新增部分。
...
设置的“声音”页可以开启安静时段，并设置开始/结束时间。安静时段内会暂停自动完成、失败和批准提示音；“试听”按钮仍会播放，方便确认声音文件和音量。
...
- 严格：默认推荐。失败只认 Codex 日志里的明确失败事件；普通文本和探索性命令不会触发失败提醒。
- 智能：在严格模式基础上，根据最后回复里的无法完成、受阻、超时等表达判断失败。
- 开发者：最敏感。智能模式之外，命令非 0 退出也会参与失败判断，适合调试。
